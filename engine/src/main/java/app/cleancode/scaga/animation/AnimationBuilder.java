...
public class AnimationBuilder {
    ...
    public Animation buildAnimation(String character, String animation, int cellCount, Duration duration, double height,
            boolean reversed) {
        String templatePath = String.format("/characters/%s/%s/[index].png", character, animation);
        Image testImage = SwingFXUtils
                .toFXImage(resourceReader.getResourceAsImage(templatePath.replace("[index]", "0")), null);
    ...
    public Animation buildAnimation(AnimationConfig config) {
        return buildAnimation(config.getCharacter(), config.getAnimation(), config.getFrames(), config.getDuration(),
                config.getHeight(), config.getReversed());
    }
}
